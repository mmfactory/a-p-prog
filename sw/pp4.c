    ...
    while ((read =  getlinex(&line, &len, sf)) != -1)
        ...
        if (line[0]!='#')
			...
			if (strcmp(read_cpu_type,cpu)==0)
				...
				if (strcmp("CF_P16F_X",read_algo_type)==0) {chip_family = CF_P16F_C; isX=1;}
...
int p16c_mass_erase (void)
    {
    if (verbose>2) flsprintf(stdout,"Mass erase p16c\n");
    putByte(0x43);
    putByte(0x00);
    getByte();
    return 0;
...
int p16e_mass_erase (void)
    {
    if (verbose>2) flsprintf(stdout,"Mass erase p16e\n");
    putByte(0x47);
    putByte(0x00);
    getByte();
    return 0;
...
int p16x_mass_erase (void)
    {
    if (verbose>2) flsprintf(stdout,"Mass erase p16x\n");
    putByte(0x48);
    putByte(0x00);
    getByte();
// exit enter program mode
    putByte(0x02);
    putByte(0x00);
    getByte();
    putByte(0x40);
...
//    for (i=0; i<10; i++) printf ("%2.2x",config_bytes[i]);
    ...
    for (i=0; i<10; i++)
    {
      if((i%2)!=0) config_bytes[i]=0x3f & config_bytes[i];
    }
    ...
    if ((chip_family==CF_P18F_A)|(chip_family==CF_P18F_B)|(chip_family==CF_P18F_D)|(chip_family==CF_P18F_E)|(chip_family==CF_P18F_F)|(chip_family==CF_P18F_Q))
        ...
        if (program==1)
            ...
            if ((chip_family==CF_P16F_C))
            {
              if(isE) p16e_mass_erase();
              else if(isX) p16x_mass_erase();
              else p16c_mass_erase();
//              printf("device ID: %x\n",prog_get_device_id());
            }
            ...
            fflush(stdout);
//            if(isE) p16c_write_single_cfg(config_bytes[9],config_bytes[8],0x800B);
            for (i=0; i<flash_size; i=i+page_size)
                {
                if (verbose>1)
                    {
        ...
        if (verify==1)
            ...
            for (i=0; i<flash_size; i=i+page_size)
                ...
                for (j=0; j<page_size; j++)
                    {
//                        printf("%4x %4x %2x %2x %2x\n",i,i+j,file_image[i+j],progmem[i+j],tdat[j]);
